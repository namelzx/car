...
//        dump($res);
    ...
    public function orderData()
    {
        ...
        clt_bis_models WHERE (location_ids like
          '" . $data['data']['bis_id'] . ",%' or
          location_ids like '%," . $data['data']['bis_id'] . ",%'
           or location_ids like '%," . $data['data']['bis_id'] . "') and models_id='" . $data['data']['models_id'] . "'limit 1 ");
        if (empty($res)) {
            return json(['status' => 304, 'data' => '', 'msg' => '该店没有这个车型']);

        }
        $data = BisService::GetServiceinfoBy($res[0]['id'], $data['todos']);
    ...
    function groupVisit($data)
    {
        ...
        try {
            ...
            foreach ($data as $v => $items) {
                $visit_list[$v] = $data[$v];
                $visit_list[$v]['chlid'] = [];
                $child = db('bis_child')->where('service_id', $visit_list[$v]->id)->select();
                for ($i = 0; $i < count($child); $i++) {
                    $dd[$v][$i] = $child[$i];
                    $dd[$v][$i]['show'] = false;
                    $ser = db('bis_chlid_service')->where('child_id', $child[$i]['id'])
//                        ->whereIn('category_id')
                        ->select();
            ...
            return json(['status' => 200, 'data' => $visit_list, 'msg' => '加载']);
        } catch (Exception $e) {
